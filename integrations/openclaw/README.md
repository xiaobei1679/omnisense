...
> 另：`omnisense-link.mjs monitor [snapshot|health|alerts|dashboard|toolHealth]` 让工作区侧观测「身体是否健康」（第 8 器官 monitor，复用内核同一份 monitor 总线契约）：`toolHealth` 给出工具管线健康（缓存命中/熔断状态/工具级 P50-P95-P99 延迟分布 + circuit_open 熔断开启告警），三层（内核 `node src/cli.mjs monitor` / 桥接 `omni-body.mjs` / 工作区）一致。
