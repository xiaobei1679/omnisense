...
> 📦 本目录是 **OmniSense** 仓库内的子包（原 `xiaobei1679/openclaw-workspace`，MIT）。
> OmniSense 通过仓库根 `integrations/openclaw/` 把它接入为「多智能体工作区」，并已在
> `config/openclaw.json.example` 注册 `omnisense-engine` 角色。
> 下方 CI 徽章指向原独立仓库；合并后统一由 OmniSense 父仓库的 Actions 持续集成。
...
OMNI_TOOL_CACHE_FILE=./.omni-tool-cache.json node scripts/omnisense-link.mjs cache   # 启用落盘持久化（跨重启续命，启动时自动载入）
node scripts/omnisense-link.mjs cache --persist-file=./.omni-tool-cache.json --flush   # 立即落盘一次
node scripts/omnisense-link.mjs cache --clear-persist   # 清空内存与磁盘
...
node scripts/omnisense-link.mjs trace --export=spans.otlp.json --export-format=otlp # 导出 OTLP/JSON（OTel-native，每个 span 注入 OTel GenAI span events：user/assistant/tool/exception；可投 Grafana Tempo/Phoenix/Jaeger）
node scripts/omnisense-link.mjs trace --baseline=<id>  # 设基线
node scripts/omnisense-link.mjs trace --regression     # 回归门禁（退化退出码 1，可接 CI）
# 监控器官：观测身体是否健康（第8器官 monitor，跨层复用内核同一份 monitor 总线契约）
node scripts/omnisense-link.mjs monitor snapshot    # 整体健康快照
node scripts/omnisense-link.mjs monitor health      # 核心健康指标
node scripts/omnisense-link.mjs monitor alerts      # 告警列表
node scripts/omnisense-link.mjs monitor dashboard   # 零依赖 HTML 仪表盘
node scripts/omnisense-link.mjs monitor toolHealth  # 工具管线健康：缓存命中 / 熔断状态 / 工具级 P50-P95-P99 延迟分布 + circuit_open 熔断开启告警
node scripts/omnisense-link.mjs monitor trendAnomalies  # 趋势异常（OLS 斜率：延迟/成功率/记忆持续爬坡或劣化）
...
node scripts/omnisense-link.mjs monitor --config-file=./my-monitor.json --scope=prod config  # 多舰队差异化阈值（按 prod profile 查询，来源标 scope）
node scripts/omnisense-link.mjs monitor thresholdHealth  # 当前测量值 vs 阈值 红黄绿着色（ok/warn/over/na）
node scripts/omnisense-link.mjs monitor --scope=llm thresholdHealth  # 按引擎 scope 过滤测量（仅 llm 引擎）+ 应用 llm scope 差异化阈值
...
node scripts/omnisense-link.mjs monitor --scope=prod thresholdAlerts  # 多舰队差异化告警（按 prod profile 产出该舰队告警）
node scripts/omnisense-link.mjs monitor healthScore     # 综合健康评分（0-100 加权 5 维度：Liveness/可靠性/阈值/异常/工具管线 + 等级 A/B/C/D/F）
node scripts/omnisense-link.mjs monitor score           # 别名：综合健康评分
node scripts/omnisense-link.mjs monitor weights          # 综合健康评分维度权重(值/归一化/来源，可用 OMNI_MONITOR_WEIGHT_* 或 --weights-file JSON 覆盖)
node scripts/omnisense-link.mjs monitor --weights-file=./my-weights.json weights  # 从 JSON 文件加载维度权重(Observability-as-Code)
```
