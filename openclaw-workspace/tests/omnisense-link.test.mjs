...
it('runLink monitor trends 跨层消费趋势基线(时间序列 + sparkline)', async () => {
  // 合并后新项目：工作区能观察身体"随时间变化的指标趋势"（每次快照落盘轻量指标点）。
  // 复用内核同一份 monitor.trends()，跨层一致。
  const r = await runLink(['monitor', 'trends']);
  assert.ok('count' in r && 'series' in r, 'trends 应含 count/series');
  assert.ok(Array.isArray(r.series.p95), 'p95 序列应为数组');
  assert.ok('sparkline' in r, '应含 sparkline');
});

it('runLink monitor trendAnomalies 跨层消费趋势异常检测(渐进退化信号)', async () => {
  // 合并后新项目：工作区能接收身体的"慢煮青蛙"式渐进退化信号（P95 爬坡/成功率漂移/记忆空转）。
  // 复用内核同一份 monitor._detectTrendAnomalies()，跨层一致。
  const r = await runLink(['monitor', 'trendAnomalies']);
  assert.ok(Array.isArray(r), 'trendAnomalies 应返回数组');
  // 未造数据时可以是空数组（zero-points），不应报错
  assert.ok(r.every(a => typeof a.level === 'string' && typeof a.type === 'string' && typeof a.message === 'string'),
    '每条告警应含 level/type/message');
});

it('runLink monitor config 跨层消费告警阈值配置(值/来源/环境变量名)', async () => {
  // 合并后新项目：工作区能查询身体的"生效告警阈值"（可用 OMNI_MONITOR_* 覆盖，避免硬编码反模式）。
  // 复用内核同一份 monitor.config()，跨层一致。
  const r = await runLink(['monitor', 'config']);
  assert.equal(r.ok, true);
  assert.ok(r.thresholds && typeof r.thresholds === 'object', 'config 应含 thresholds');
  const sf = r.thresholds.spikeFactor;
  assert.ok(sf && typeof sf.value === 'number', 'spikeFactor 应有数值');
  assert.equal(sf.envKey, 'OMNI_MONITOR_SPIKE_FACTOR', '应暴露对应环境变量名');
  assert.ok(['default', 'env', 'opts', 'file'].includes(sf.source), 'source 应为 default/env/opts/file');
  assert.ok(typeof r.count === 'number' && Array.isArray(r.overrides), '应含 overrides/count');
});

it('runLink monitor thresholdHealth 跨层消费阈值健康(当前值 vs 阈值 红黄绿)', async () => {
  // 合并后新项目：工作区能看身体的"每个阈值现在离超标多远"（红黄绿着色的数据源）。
  // 复用内核同一份 monitor.thresholdHealth()，跨层一致。
  const r = await runLink(['monitor', 'thresholdHealth']);
  assert.equal(r.ok, true);
  assert.ok(Array.isArray(r.items) && r.items.length >= 11, 'thresholdHealth 应含 11 项阈值');
  assert.ok(r.summary && typeof r.summary === 'object', '应含 summary(ok/warn/over/na)');
  const sf = r.items.find(i => i.key === 'spikeFactor');
...
it('runLink monitor healthScore 跨层消费综合健康评分(0-100 + 等级 A/B/C/D/F)', async () => {
  // 合并后新项目：工作区能看身体的"一个分数"(加权汇总 Liveness/成功率/阈值/异常/工具管线，借鉴 Nobl9 Composite SLO
  // 与 New Relic 健康分)。复用内核同一份 monitor.healthScore()，跨层一致。
  const r = await runLink(['monitor', 'healthScore']);
  assert.equal(r.ok, true, '应返回综合健康评分');
  assert.ok('score' in r, '应含 score(0-100 或 null)');
  ...
  assert.ok(Array.isArray(r.dimensions) && r.dimensions.length === 5, '应含 5 个加权维度');
  assert.ok(Array.isArray(r.issues), '应含关键问题数组');
  // 用别名 score 也应一致返回结构
  const r2 = await runLink(['monitor', 'score']);
...
it('runLink monitor --config-file 跨层从 JSON 文件加载阈值(Observability-as-Code)', async () => {
  ...
  // 复用内核同一份 loadConfigFile + config()，跨层一致。
  const cfgPath = join(_td, `monitor-config-${Math.random().toString(36).slice(2)}.json`);
  writeFileSync(cfgPath, JSON.stringify({ spikeFactor: 7, memBulk: 3 }), 'utf8');
  const r = await runLink(['monitor', '--config-file=' + cfgPath, 'config']);
  assert.equal(r.ok, true);
  assert.equal(r.thresholds.spikeFactor.value, 7, '跨层：JSON 文件覆盖 spikeFactor');
  assert.equal(r.thresholds.spikeFactor.source, 'file', '跨层：source 应为 file');
  assert.equal(r.thresholds.memBulk.value, 3, '跨层：JSON 文件覆盖 memBulk');
  assert.equal(r.configFile, cfgPath, '跨层：应暴露配置文件路径');
  assert.equal(r.configFileLoaded, true);
  assert.equal(r.count, 2, '跨层：应识别 2 项被覆盖');
});
