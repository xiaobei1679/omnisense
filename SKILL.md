...
node "{SKILL_DIR}/src/cli.mjs" trace --export=spans.otlp.json --export-format=otlp  # 导出 OTLP/JSON（OTel-native，每个 span 注入 OTel GenAI span events：user/assistant/tool/exception；可投 Grafana Tempo/Phoenix/Jaeger）
```
...
- **可视化仪表盘 `dashboard`**：零依赖静态 HTML（驾驶舱风格），含器官/舰队健康/延迟 sparkline/记忆健康/工具管线健康/**阈值配置**(展示来源文件与每项来源)/**可推送告警清单**(Alertmanager 形状)/活动/告警/运行时间线。
- **可推送告警清单 `monitor --threshold-alerts`（Alertmanager-ready，常驻迭代能力）**：把 thresholdHealth 中"超标/关注"的阈值项转成可直接提交 Prometheus Alertmanager 的告警 payload——`labels{alertname=omnisense_threshold_<key>,severity,monitor,key,status}` + `annotations{summary,description,current,threshold,source}` + 同一 key 稳定的 `fingerprint`；状态映射 `over→critical`、`warn→warning`、`ok/na→none`（对齐 Alertmanager severity 标签）。离线不主动外发，仅产出形状一致的 payload，接入方直接 `POST /api/v2/alerts` 即可（借鉴 Prometheus Alertmanager 告警数据模型）。
- **综合健康评分 `monitor --score` / `monitor --health-score`（本轮新增常驻迭代能力）**：把分散观测信号加权汇总成一个 0-100 综合健康分 + 等级(A/B/C/D/F)——5 维度 Liveness 存活(0.25)＋Reliability 可靠性(0.25)＋Threshold 阈值合规(0.20)＋Anomalies 异常(0.15)＋Tool 工具管线(0.15)，`score=round(100×Σ weight·subScore)`，A≥90/B≥75/C≥60/D≥40/F<40，并聚合 Top 问题清单（最多 12 条，na/未知绝不伪造读数）；无运行轨迹时 `score=null`+等级`N/A`+状态`unknown`（诚实降级）。CLI `monitor --score`，工作区 `omnisense-link monitor healthScore`/`score` 跨层复用同一份能力。借鉴 Nobl9 Composite SLO（加权 rollup、非等权重）与 New Relic Operational Health Score（0-100 加权）。
