...
- **可视化仪表盘 `dashboard`**：零依赖静态 HTML（驾驶舱风格），含器官/舰队健康/延迟趋势 sparkline/记忆健康/**工具管线健康**/活动/告警/运行时间线/阈值配置（当前值 vs 阈值 红黄绿 + **可推送告警清单**）。
...
node src/cli.mjs dashboard                    # 生成零依赖静态 HTML 仪表盘（含「阈值配置」区块，展示配置文件路径与当前值 vs 阈值着色 + 可推送告警清单）
