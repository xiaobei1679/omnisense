...
node src/cli.mjs live --ticks=3 --speak     # 生命循环：自驱地感知→思考→动手→说话→移动
...
OmniSense 每次跑 `agent` / `multiagent`（含 `live` 生命循环、watch 自主编排派发的 agent）都会把整次运行**自动记录为可回放 trace**——离线落盘到 `.omni-traces.json`，无需任何外部平台（LangSmith/AgentOps 类 SaaS 不进仓库、数据主权自持）。
...
- 每步一个 **child span**（`gen_ai.operation.name = execute_tool`，携带 `gen_ai.tool.name` / `call.arguments` / `call.result` 或 `error.type` + `error.message`，`status.code` 成功=1/失败=2）。
- 时间用纳秒级 Unix 字符串，属性值按 OTLP 类型化（`stringValue`/`boolValue`），可直接 `POST` 到 OTel Collector 的 `/v1/traces`。
...
区别于 BabyAGI：OmniSense **离线即可自驱**——每轮 (1) 感知环境 (2) 从议程自生成意图（离线确定性轮转 +
跳过需结构化参数的 `hand` 技能）(3) 用自身能力卡 `skillResolve` 把意图映射到最佳器官/方法 (4) `skillDispatch`
真正执行，并记录委派结果。全程**零网络、零挂起、可离线自活**。

```js
const omni = OmniSense.create();
// 身体自主决定每轮做什么（默认议程离线安全：思考/记忆/规划/回顾），每轮基于能力卡选器官
await omni.autopilot({ ticks: 3 });
// 自定义议程（同样离线）：每个意图都会被 skillResolve 映射到最佳器官
await omni.autopilot({ ticks: 2, agenda: ['思考当前环境', '规划下一步行动'] });
```

```bash
node src/cli.mjs autopilot --ticks=3          # 身体自驱决策（基于能力卡 skillResolve）
node integrations/openclaw/omni-body.mjs autopilot '{"ticks":2}' --json   # 桥接层
node openclaw-workspace/scripts/omnisense-link.mjs autopilot 2 --json      # 工作区侧驱动身体自主循环
```
