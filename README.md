...
- **可视化仪表盘 `dashboard`**：零依赖静态 HTML（驾驶舱风格），含器官/舰队健康/延迟趋势 sparkline/记忆健康/**工具管线健康**/活动/告警/运行时间线/阈值配置（当前值 vs 阈值 红黄绿 + **可推送告警清单**）＋ **综合健康评分区块（0-100 分 + 等级 A/B/C/D/F + 状态色 + Top 问题清单）**。
