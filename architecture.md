...
`tracer` 回答"身体**做了什么**"，而 `monitor` 回答"身体**现在健康吗**"——这是 Agent 可观测性的第二支柱（Industry 三支柱：Traces / Metrics / Evaluations，对齐 LangSmith / Langfuse / Helicone / Arize / CloudWatch GenAI）。`monitor` 经 `bus.register('monitor', method, fn)` 暴露 **21 个总线方法**（核心 6：snapshot/alerts/dashboard/statusGrid/memoryHealth/anomalies；新增 15：recentRuns/latency/toolHealth/recordMetric/checkAlerts/collect/trendAnomalies/trends/config/thresholdHealth/thresholdAlerts/alertables/healthScore/score/weights），把身体各子系统（眼/耳/脑/手/感知/脚 + 记忆 + 工具管线）的健康聚合为一份零依赖 HTML 仪表盘（`monitor dashboard`），CLI `monitor --summary/--tools/--anomalies/--grid/--trends/--config/--threshold-health/--threshold-alerts/--score/--health-score` 取结构化数据、serve `GET /monitor` 暴露。`healthScore()` 把分散的观测信号加权汇总成一个 0-100 综合健康分 + 等级(A/B/C/D/F)（借鉴 Nobl9 Composite SLO 与 New Relic Operational Health Score）。
...
- **可调告警阈值（config，v6.1.0 新增，本轮扩展 JSON 文件源）**：全部告警/异常/网格/趋势判定阈值不再硬编码，经 `THRESHOLD_SPEC`（11 个 key→[环境变量名, 默认值, 说明]）集中声明，可由三路覆盖——构造 `opts.thresholds` > `OMNI_MONITOR_*` 环境变量 > **JSON 配置文件**（`--config-file=<path>` / `OMNI_MONITOR_CONFIG` / 默认 `~/.omnisense/monitor.json`，本轮新增，Observability-as-Code）> 内置默认；`resolveThreshold()` 按 opts > env > file > default 优先级解析，每个阈值可溯源（source=default/env/file/opts），非法值自动回退默认，未知键被忽略、文件缺失静默降级。`config()` 返回全量阈值、来源与配置文件路径，`dashboard` 增「阈值配置」区块（展示来源文件，借鉴 Grafana/Prometheus「阈值即配置、纳入版本控制」的 Observability-as-Code 实践 + Grafana 动态阈值，规避硬编码阈值反模式）。
- **多舰队差异化阈值（scoped thresholds，本轮新增常驻迭代能力）**：同一份监控要为不同「引擎(llm/autopilot/local/…)」或不同「环境 profile(prod/staging/…)」用**不同告警阈值**——配置 JSON 文件可携带 `scopes` 映射（`{ prod: { inactiveMs: 1000 }, llm: { inactiveMs: 1000 } }`），按 scope 分组覆盖。`config` / `thresholdHealth` / `thresholdAlerts` 均接受 `--scope=<name>`：`scope` 是已知引擎名时测量**仅限该引擎的 runs**（engineScope=true，实现"按引擎分组"的延迟/存活差异化观测）；`scope` 是环境 profile 时测量保持全局、仅应用该 scope 的阈值覆盖（profile 只改变"用什么阈值判"，不改变"谁来测"）。优先级 **opts > env > scope > file > default**，来源诚实标注 `source:'scope'`；`config()` 回显当前 `scope` 并列出 `availableScopes`（便于"同一份监控、不同 fleet 不同告警阈值"）。借鉴 100+ Agent 生产中 per-Agent-type 差异化阈值（不同 agent 类型用不同 P95 阈值，而非全局一刀切：https://www.askmarvin.ai/blog/monitoring-100-agents）、kaxo.io 的 per-agent 可观测性（每 agent 独立阈值/仪表盘：https://www.kaxo.io/）与 Prometheus/Grafana 按环境 tier 分级告警（https://prometheus.io/docs/alerting/latest/notification_examples/）——即 Observability-as-Code 的"多舰队实例化"：同一套监控配置，不同 fleet 用不同阈值，且每个阈值变更都是一次可追溯的 PR（https://grafana.com/docs/grafana/next/alerting/set-up/provision-alerting-resources/file-provisioning/）。
- **阈值健康着色（thresholdHealth，v7.1.0 新增）**：把"告警阈值"从纯配置升级为"可一眼看懂的健康视图"——`thresholdHealth()` 用 `linRegSlope`/百分位等算子实时测出 11 项当前值（无活动时长 idleMs / 延迟突增倍数 spikeRatio / 记忆陈旧度 maxMemAgeMs / 记忆批量增长 memBulk / 引擎 liveness 健康-降级窗口 / 5 类趋势斜率 trendSlopeP95·Success·MemGrow·MemIdle·Fleet），与 `THRESHOLD_SPEC` 对应阈值逐项比对，输出 `status ∈ {ok, warn, over, na}`（na=数据不足绝不伪造读数），每项再映射 `severity`（over→critical / warn→warning / 其余→none，对齐 Prometheus Alertmanager severity 标签）。`dashboard`「阈值配置」区块对每个阈值同时渲染**当前值 vs 阈值**并红黄绿着色（绿=ok / 黄=warn / 红=over，Base=健康、warning=黄、critical=红，借鉴 Grafana 阈值阶梯着色 `thresholds` 的 Base/warning/critical 三态约定：https://grafana.com/docs/grafana/latest/visualizations/panels-visualizations/configure-thresholds/），CLI `monitor --threshold-health` + 工作区 `omnisense-link monitor thresholdHealth` 跨层复用同一份能力。
- **可推送告警清单（thresholdAlerts / alertables，常驻迭代能力）**：把"超标/关注"的阈值项转成可直接提交 Prometheus Alertmanager 的告警 payload（借鉴 Alertmanager 告警数据模型：labels + annotations + 稳定 fingerprint）——`labels{alertname=omnisense_threshold_<key>,severity,monitor,key,status}`、`annotations{summary,description,current,threshold,source}`、同一 key 跨运行稳定的 `fingerprint`（内置 crypto sha1，用于告警去重聚合）；`over→critical`、`warn→warning`、`ok/na→none`，绝不伪造告警。`dashboard` 阈值区块附「可推送告警清单」子块（离线不发送，仅供对接方 `POST /api/v2/alerts`）；CLI `monitor --threshold-alerts` + 工作区 `omnisense-link monitor thresholdAlerts` 跨层复用同一份能力。
- **综合健康评分（healthScore / score，本轮新增常驻迭代能力）**：把分散的观测信号加权汇总成**一个 0-100 综合健康分 + 等级(A/B/C/D/F)**，一眼看清整体健康——5 个加权维度：**Liveness 存活(0.25)**＋**Reliability 可靠性(0.25，SLO 成功率取自 tracer 运行轨迹)**＋**Threshold 阈值合规(0.20，当前值 vs 阈值，不含 liveness*Ms 避免双重计数)**＋**Anomalies 异常(0.15)**＋**Tool 工具管线(0.15，缓存命中/熔断状态/工具延迟分布，任一熔断开启即 0)**。`score = round(100 × Σ weight·subScore)`，等级 A≥90/B≥75/C≥60/D≥40/F<40，并聚合 Top 问题清单（按严重度排序，最多 12 条，na/未知维度绝不伪造读数）；无运行轨迹时 `score=null` + 等级 `N/A` + 状态 `unknown`（诚实降级）。`dashboard` 新增「综合健康评分」区块（介于 KPI 网格与舰队区块之间）；CLI `monitor --score`/`--health-score` + 工作区 `omnisense-link monitor healthScore`/`score` 跨层复用同一份能力。**维度权重可配置（本轮新增 `weights` 能力）**：5 维度权重（默认 Liveness 0.25 / Reliability 0.25 / Threshold 0.20 / Anomalies 0.15 / Tool 0.15）可经构造 `opts.weights` > `OMNI_MONITOR_WEIGHT_*` 环境变量 > **JSON 权重文件**（`--weights-file=<path>` / `OMNI_MONITOR_WEIGHTS` / 默认 `~/.omnisense/monitor-weights.json`，Observability-as-Code）> 内置默认 四路覆盖，`resolveWeight()` 按 opts > env > file > default 解析并诚实标注 `source`，`weights()` 返回每维权重/归一化值/来源；**计分前自动按权重和归一化**，使分数恒在 0-100，不受"权重之和≠1"影响（复用 v7.0.0 Observability-as-Code 思路，让"健康评分怎么算"也纳入版本控制）。借鉴 Nobl9 **Composite SLOs**（加权 rollup、非等权重：https://www.nobl9.com/features/composite-service-level-objectives · https://docs.nobl9.com/guides/slo-guides/composite-slos-use-cases）与 New Relic/Vortex IQ **Operational Health Score**（0-100 加权：Apdex/错误率/事件数/SLO 合规度：https://www.newrelic.com/blog/nerdlog/operational-health-score）。
- **异常检测（detectAnomalies）**：覆盖 circuit_open（熔断开启）、记忆健康劣化、引擎延迟突增、`run` 失败率超标等规则，命中即发 warning 告警，可接 `monitor --anomalies` / `checkAlerts`。
- 工作区侧经 `openclaw-workspace/scripts/omnisense-link.mjs monitor [snapshot|health|alerts|dashboard|toolHealth|trends|trendAnomalies|config|thresholdHealth|thresholdAlerts|alertables|healthScore|score|weights]` 跨层复用内核同一份 monitor（同源 bus 契约），三层（内核 `node src/cli.mjs monitor` / 桥接 `omni-body.mjs` / 工作区）一致。
...
src/
  ...
  core/
    ...
    tools.mjs         工具执行器（web_fetch/文件/calc/now/记忆/热搜… 安全白名单）；executeTool 统一入口复用 breaker.mjs 的 TTL 缓存 + 熔断器（声明式：工具定义加 cacheTtl/circuit 即启用，覆盖 web_fetch/summarize_url/hot_topics，避免重复联网与反复超时）；缓存/熔断支持**落盘持久化**（OMNI_TOOL_CACHE_FILE / --persist-file，零依赖 JSON，跨重启续命）
  ...
  modules/
    ...
    monitor.mjs       监控器官（第 8 器官）：统一观测身体是否健康（snapshot/alerts/dashboard + 20 个总线方法：含 healthScore 综合健康评分(0-100 加权 Liveness/可靠性/阈值/异常/工具管线 5 维度 + 等级 A/B/C/D/F) + toolHealth 工具管线健康：缓存命中/熔断状态/工具级 P50-P95-P99 延迟分布 + circuit_open 熔断开启告警 + 趋势异常(OLS 斜率) + 可调告警阈值(config：JSON 文件/OMNI_MONITOR_* 环境变量/opts 覆盖，值可溯源 default/env/file/opts) + 阈值健康着色(ok/warn/over/na) + 可推送告警清单(Alertmanager 形状)；借鉴 LangSmith/Langfuse/Helicone/Arize/CloudWatch GenAI 三支柱 + Nobl9 Composite SLO + New Relic 健康分 + OpenLIT「工具可靠性是 agent 延迟的暗物质」+ dev.to AgentCircuitBreaker + Grafana/Prometheus 动态阈值）
